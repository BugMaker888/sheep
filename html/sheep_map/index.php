...
<html lang="zh-CN">
    ...
    <body>
        <div id="info">
            ...
            <a id="auto_solve" href="javascript:void(0)">【自动解答】</a>
            <a id="single_step_solve" href="javascript:void(0)">【单步解答】</a>
        </div>
        ...
        <script type="module">
            ...
            function print_block_info(block_data, is_remove) {
                var info = `层数:${block_data['layerNum']}, 坐标:(${block_data['rolNum']}, ${block_data['rowNum']}), 类型:${block_data['type']}`
                if (is_remove) {
                    info = "移除方块 >> " + info;
                }
                console.log(info);
            ...
            function auto_solve() {
                setTimeout(function() {
                    ...
                    var interval = setInterval(function() {
                        if (index >= map_data['oprations'].length) {
                            clearInterval(interval);
                            return;
                        }
                        var block_id = map_data['oprations'][index];
                        var block_object = block_objects[block_id];
                        ...
                        index += 1
                        if (index < map_data['oprations'].length) {
                            block_id = map_data['oprations'][index];
